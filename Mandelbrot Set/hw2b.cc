...
int main(int argc, char** argv) {
    ...
    for (int row = rank; row < height; row += size) {
        ...
        for (int i = 0; i < width; i += 16) {
            ...
            _mm512_mask_cvtepi64_storeu_epi32(&image[row * width + i], mask & 0xFF, repeats_low);
            _mm512_mask_cvtepi64_storeu_epi32(&image[row * width + i + 8], mask >> 8, repeats_high);
        }
    }
    MPI_Reduce(image, ans, height*width, MPI_INT, MPI_BOR, 0, MPI_COMM_WORLD);
    
    if (rank == 0) {
        write_png(filename, iters, width, height, ans);
    }
